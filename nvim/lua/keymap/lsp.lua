...
local function navigation(config)
	if not config.next_error.disabled then
		vim.keymap.set("n", config.next_error.keybind, function()
			vim.diagnostic.jump({ count = 1, float = true })
		end, { desc = "Jump to next error" })
	end
	if not config.prev_error.disabled then
		vim.keymap.set("n", config.prev_error.keybind, function()
			vim.diagnostic.jump({ count = -1, float = true })
	...
	if not config.toggle_header.disabled then
		vim.keymap.set("n", config.toggle_header.keybind, function()
			...
			if not found then
				vim.notify(
					"Cannot swap header-source files outside of C/C++",
					4,
	...
	if not config.goto_definition.disabled then
		vim.keymap.set("n", config.goto_definition.keybind, function()
			vim.lsp.buf.definition()
		end, { desc = "Go to definition" })
	end

	if not config.goto_declaration.disabled then
		vim.keymap.set("n", config.goto_declaration.keybind, function()
			vim.lsp.buf.declaration()
		end, { desc = "Go to declaration" })
	end

	if not config.goto_type_definition.disabled then
		vim.keymap.set("n", config.goto_type_definition.keybind, function()
			vim.lsp.buf.type_definition()
		end, { desc = "Go to type definition" })
	end

	if not config.goto_implementation.disabled then
		vim.keymap.set("n", config.goto_implementation.keybind, function()
...
local function actions(config)
	if not config.hover_info.disabled then
		vim.keymap.set("n", config.hover_info.keybind, function()
			vim.lsp.buf.hover()
		end, { desc = "Open window with information of hovered element" })
	end

	if not config.rename.disabled then
		vim.keymap.set("n", config.rename.keybind, function()
			vim.lsp.buf.rename()
		end, { desc = "Rename hovered element" })
	end

	if not config.code_action.disabled then
		vim.keymap.set("n", config.code_action.keybind, function()
			vim.lsp.buf.code_action()
		end, { desc = "Attempt to fix problem under hover" })
	end
end
